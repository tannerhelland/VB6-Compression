...
Private Declare Function compress Lib "zlib-ng" Alias "_compress@16" (ByVal ptrToDestBuffer As Long, ByRef dstLen As Long, ByVal ptrToSrcBuffer As Long, ByVal srcLen As Long) As Long
Private Declare Function compress2 Lib "zlib-ng" Alias "_compress2@20" (ByVal ptrDstBuffer As Long, ByRef dstLen As Long, ByVal ptrSrcBuffer As Any, ByVal srcLen As Long, ByVal cmpLevel As Long) As Long
Private Declare Function uncompress Lib "zlib-ng" Alias "_uncompress@16" (ByVal ptrToDestBuffer As Long, ByRef dstLen As Long, ByVal ptrToSrcBuffer As Long, ByVal srcLen As Long) As Long
Private Declare Function zlibVersion Lib "zlib-ng" Alias "_zlibVersion@0" () As Long
...
Public Function InitializeZLibNG(ByRef pathToDLLFolder As String) As Boolean
    ...
    InitializeZLibNG = CBool(m_ZLibHandle <> 0)
    
    If (Not InitializeZLibNG) Then
        Debug.Print "WARNING!  LoadLibrary failed to load zLib-ng.  Last DLL error: " & Err.LastDllError
...
Public Function IsZLibNGAvailable() As Boolean
    IsZLibNGAvailable = CBool(m_ZLibHandle <> 0)
End Function

'Return the current zLib version
Public Function GetZLibNGVersion() As String
    ...
    If (m_ZLibHandle <> 0) Then
        ...
        GetZLibNGVersion = VBHacks.ConvertCharPointerToVBString(ptrZLibVer, False, 255)
        
    Else
        GetZLibNGVersion = vbNullString
    End If
...
Public Function ZlibNGCompressArray(ByRef dstArray() As Byte, ByVal ptrToSrcData As Long, ByVal srcDataSize As Long, Optional ByVal dstArrayIsReady As Boolean = False, Optional ByVal dstArraySizeInBytes As Long = 0, Optional ByVal compressionLevel As Long = -1) As Long
    ...
    If CBool(compress2(VarPtr(dstArray(0)), dstArraySizeInBytes, ptrToSrcData, srcDataSize, compressionLevel) = ZLIB_OK) Then
        ZlibNGCompressArray = dstArraySizeInBytes
    Else
        ZlibNGCompressArray = 0
    End If
...
'         (in bytes, 1-based).
Public Function ZlibNGCompressNakedPointers(ByVal dstPointer As Long, ByRef dstLength As Long, ByVal srcPointer As Long, ByVal srcLength As Long, Optional ByVal compressionLevel As Long = -1) As Boolean
    If (compressionLevel < ZLIB_MIN_CLEVEL) Then
        compressionLevel = ZLIB_DEFAULT_CLEVEL
    ElseIf (compressionLevel > ZLIB_MAX_CLEVEL) Then
        compressionLevel = ZLIB_MAX_CLEVEL
    End If
    ZlibNGCompressNakedPointers = CBool(compress2(dstPointer, dstLength, srcPointer, srcLength, compressionLevel) = ZLIB_OK)
End Function
...
Public Function ZlibNGDecompressArray(ByRef dstArray() As Byte, ByVal ptrToSrcData As Long, ByVal srcDataSize As Long, ByRef knownUncompressedSize As Long, Optional ByVal dstArrayIsReady As Boolean = False) As Long
    ...
    ZlibNGDecompressArray = CBool(uncompress(VarPtr(dstArray(0)), knownUncompressedSize, ptrToSrcData, srcDataSize) = ZLIB_OK)
...
'         to the destination buffer, in bytes (1-based).
Public Function ZlibNGDecompress_UnsafePtr(ByVal ptrToDstBuffer As Long, ByRef knownUncompressedSize As Long, ByVal ptrToSrcData As Long, ByVal srcDataSize As Long) As Boolean
    ZlibNGDecompress_UnsafePtr = CBool(uncompress(ptrToDstBuffer, knownUncompressedSize, ptrToSrcData, srcDataSize) = ZLIB_OK)
End Function
...
Public Function ZlibNGGetMaxCompressedSize(ByVal srcSize As Long) As Long
    ZlibNGGetMaxCompressedSize = srcSize + (CSng(srcSize) * 0.01) + 12
End Function

Public Function ZLibNG_GetDefaultCompressionLevel() As Long
    ZLibNG_GetDefaultCompressionLevel = ZLIB_DEFAULT_CLEVEL
End Function

Public Function ZLibNG_GetMinCompressionLevel() As Long
    ZLibNG_GetMinCompressionLevel = ZLIB_MIN_CLEVEL
End Function

Public Function ZLibNG_GetMaxCompressionLevel() As Long
    ZLibNG_GetMaxCompressionLevel = ZLIB_MAX_CLEVEL
End Function
