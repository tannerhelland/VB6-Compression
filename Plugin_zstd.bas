...
Public Function InitializeZStd(ByRef pathToDLLFolder As String) As Boolean
    ...
    If InitializeZStd Then
        ...
        Debug.Print "zstd is ready.  Max compression level supported: " & CStr(m_ZstdCompressLevelMax)
    Else
        Debug.Print "WARNING!  LoadLibrary failed to load zstd.  Last DLL error: " & Err.LastDllError
